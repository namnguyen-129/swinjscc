...
class Channel(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # Thay thế config.device
    ...
    def gaussian_noise_layer(self, input_layer, std, name=None):
        ...
        return input_layer.to(device) + noise
    ...
    def forward(self, input, chan_param, avg_pwr=False): # input đây là x or feature 
        ...
        if self.chan_type == 1 or self.chan_type == 'AWGN':
            noise = (channel_output.to(device) - channel_tx.to(device)).detach() # tách noise 
            ...
            channel_tx = channel_tx.to(device) + noise
            if avg_pwr:
